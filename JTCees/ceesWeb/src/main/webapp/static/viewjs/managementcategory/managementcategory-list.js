...
function queryManagementCategory() {
	$('#ordertable').datagrid('options').url = '/managementCategory/queryManagementCategory.json';
	// 为loadGrid()添加参数
					...
					onConfirm : function() {
						jQuery
								.ajax({
									type : 'POST',
									async: false,
									url : '/managementCategory/delManagementCategory.json?id='
											+ id,
...
function toAddManagementCategory(){
    window.location = '/managementCategory/addManagementCategory';
}
...
function saveManagementCategory(){
	...
	jQuery.ajax({
		...
		url : '/managementCategory/insertManagementCategory.json',
		success: function (data){
			if(data.flag == 0){
