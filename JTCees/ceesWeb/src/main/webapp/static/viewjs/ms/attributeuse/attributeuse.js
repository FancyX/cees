...
function exportInfo(){
    jqm.confirm({
        ...
        onConfirm:function(){
            ...
            var action = m.action;
            m.action = "/attributeuse/exportExcel";
...
function delAttributes(){
    ...
    jqm.confirm({
        ...
        onConfirm:function(){
            ...
            jQuery.ajax({
                type : 'POST',
                url : '/attributeuse/invalidAttribute.json',
                data : {
...
function queryAttributeList() {
    $('#ordertable').datagrid('options').url = '/attributeuse/loadAttributebyclassify.json';
//    //为loadGrid()添加参数
    ...
    if(queryParams.attributeCode==-1&&queryParams.attributeStatus==-1/*&&queryParams.attrDownStatus==-1*/
