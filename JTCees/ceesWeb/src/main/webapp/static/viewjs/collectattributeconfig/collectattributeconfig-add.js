$(function(){
    ...
    function alreadyLoadGrid(){
        $('#ordertable1').datagrid({
            ...
            loadMsg:'数据加载中......',
            columns:[[
				{field:'attributeCode',title:'属性编码',width:100},
...
function setAllChecked(){
    ...
    function queryAttributeList(){
        ...
        $('#ordertable').datagrid('options').url='/collectattributeconfig/queryattribute.json';
        //为loadGrid()添加参数
        var queryParams = $('#ordertable').datagrid('options').queryParams;
        queryParams.attributeType = $('#attributeType').val();
        queryParams.thirdgroupNo = thirdgroupNo;
        //重新加载datagrid的数据
 ...
 // 根据三级品类查询商品信息（二级）
    ...
    function queryAttributeConfigIsExist(){
        ...
        jQuery.ajax({
                ...
                url : '/collectattributeconfig/queryAttributeConfigIsExist.json',
                success : function(data){
                    if(data.flag==false){
                         Commons.showWarn("属性编码为："+data.result+"已经存在，请删除!");
                         flagRes = false;
                    }
                }
            });
	...
	function saveCollectAC(){
    	...
    	if(rowData.length>=1){
            ...
            attrContJsonStr = '';
            for(var i= 0;i<rowData.length;i++){
                attrContJsonStr += '{"attributeType":"'+rowData[i].attributeType+'","attributeName":"'+rowData[i].attributeName+'"}_';
            }
        ...
        var rndJqmId = 'addCollectAttrConfigLoading';
        jqm.loading({
            jqmId: rndJqmId,
            size: 48,
            content: ''
		...
		jQuery.ajax({
                ...
                url : '/collectattributeconfig/addCollectAttributeConfig',
