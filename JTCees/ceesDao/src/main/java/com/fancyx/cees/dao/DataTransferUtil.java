...
//		Iterator<K> keys = sourceMap.keySet().iterator();
        Iterator<Entry<K,V>> entrys = sourceMap.entrySet().iterator();
		while (entrys.hasNext()) {
			...
			if (Log.isDebugEnabled()) {
                Log.debug("propertie=" + propertie);
			}
			Class<?> cls = PropertyUtils.getPropertyDescriptor(obj, propertie).getPropertyType();
			if (Log.isDebugEnabled()) {
                Log.debug("HashMap Data Type=" + value.getClass().getName());
                Log.debug("Bean Data Type=" + cls.getName());
			}
